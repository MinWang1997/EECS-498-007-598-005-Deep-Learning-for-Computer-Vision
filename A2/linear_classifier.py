...
def svm_loss_vectorized(W, X, y, reg):
  ...
  scores = torch.mm(X,W) #scores matrix (N,C)
  scores_correct_class = scores[torch.arange(num_train), y]#select correct scores (N,)
 
  margin = torch.maximum(torch.zeros_like(scores), (scores - scores_correct_class.reshape(-1,1) + 1)) # compute margin (N,C)
  margin[torch.arange(num_train), y] = 0 #set margin at y_i = 0 so that it will be excluded during summation (j!=y_i)
  
  loss = torch.mean(torch.sum(margin, axis=1)) #compute L_i(N,) for each sample x_i and compute mean of L_i 
  ...
  #############################################################################
  # Replace "pass" statement with your code
  margin_binary = margin # (N,C)
  margin_binary[margin > 0] = 1 # indicator

  margin_binary_yi = torch.sum(margin_binary, axis=1) #(N,) 
  margin_binary[torch.arange(num_train), y] = - margin_binary_yi #update margin_binary at j=y_i

  dW = torch.mm(X.T, margin_binary)#X.shape(N, D)->X.T.shape(D, N) -> dW.shape (D, C)
  dW /= num_train
  dW += 2*reg*W
  #############################################################################
  #                             END OF YOUR CODE                              #
...
def test_one_param_set(cls, data_dict, lr, reg, num_iters=2000):
  ...
  train_acc = torch.eq(data_dict['y_train'], y_pred_train).sum() / data_dict['y_train'].shape[0]
  

  y_pred_val = cls.predict(data_dict['X_val']) 
  val_acc = torch.eq(data_dict['y_val'], y_pred_val).sum() / data_dict['y_val'].shape[0]
...
def softmax_loss_naive(W, X, y, reg):
  ...
  for i in range(num_train):
    scores = W.t().mv(X[i])
    scores -= torch.max(scores)#shift max of scores to 0 for mumeric stability

    correct_class_score = scores[y[i]]

    #compute loss
    likelihood = torch.exp(correct_class_score) /torch.exp(scores).sum()
    loss += - torch.log(likelihood)

    #compute dW
    for j in range(num_classes):
      if j == y[i]:
        dW[:, j] += torch.exp(correct_class_score) * X[i] /torch.exp(scores).sum() - X[i]
      else:
        dW[:, j] += torch.exp(scores[j])* X[i] /torch.exp(scores).sum()
...
def softmax_loss_vectorized(W, X, y, reg):
  ...
  scores = torch.mm(X,W) #scores matrix (N,C)
  max_f_j, _ = torch.max(scores, dim=1, keepdim= True) #shape (N,1)
  scores -= max_f_j#shift max of scores to 0 for mumeric stability

  scores_correct_class = scores[torch.arange(num_train), y]#select correct scores (N,)
  
  #compute loss
  likelihood_correct_class = torch.exp(scores_correct_class) / torch.exp(scores).sum(dim=1)  
  loss =  - torch.log(likelihood_correct_class).sum()#sum over N samples 
  ...
  #compute dW
  likelihood_all = torch.exp(scores) / torch.exp(scores).sum(dim=1, keepdim=True) # (N,C) 

  likelihood_all[torch.arange(num_train), y] -= 1 #update at j=y_i (likelihood -1)

  dW = torch.mm(X.T, likelihood_all)#X.shape(N, D)->X.T.shape(D, N) -> dW.shape (D, C)
