...
def nn_forward_backward(params, X, y=None, reg=0.0):
    ...
    #shift max of scores to 0 for mumeric stability
    ...
    likelihood_correct_class = likelihood_all[torch.arange(N), y]#select correct scores (N,)


    loss =  - torch.log(likelihood_correct_class).sum()#sum over N samples 

    loss /= N
    loss += reg * (torch.sum(W1 ** 2)+ torch.sum(W2 ** 2)) # Add regularization to the loss.
    ...
    #compute grad_dl_dscores
    grad_dl_dscores = likelihood_all
    grad_dl_dscores[torch.arange(N), y] -= 1 #update at j=y_i (likelihood -1)
    grad_dl_dscores /= N


    #2nd layer
    grads['b2'] = grad_dl_dscores.sum(axis=0)

    grad_dl_dh1 = torch.mm(grad_dl_dscores, W2.T)

    grads['W2'] = torch.mm(h1.T, grad_dl_dscores) + 2 * reg * W2

    #1st layer
    grad_dl_dz1 = grad_dl_dh1

    #implicit multiplication of grad_ReLU (If x_i <=0 (i.e. y==0))
    grad_dl_dz1[h1 == 0] = 0

    grads['W1'] = torch.mm(X.T, grad_dl_dz1) + 2 * reg * W1
    grads['b1'] = grad_dl_dz1.sum(axis=0)
...
def find_best_net(data_dict, get_param_set_fn):
  ...
  #get list of learning rate, hidden size, regulation strength and learning rate decays
  lrs, hss, regs, lr_decays = get_param_set_fn()
    ...
    for hs in hss:
      for reg in regs:
        for lrd in lr_decays:
          print('train with learning rate: {}, hidden size: {},regularization: {}, learning rate decay : {}'.format(lr,hs,reg,lrd))
          ...
          stat_dict[(lr,hs,reg,lrd)] = stats

          print('train accuracy', stats['train_acc_history'][-1])
          print('val accuracy', stats['val_acc_history'][-1])
          train_val_gap = stats['train_acc_history'][-1] - stats['val_acc_history'][-1] 
          print('train_val_gap', train_val_gap)
          ...
          if max(stats['val_acc_history']) > best_val_acc:
            if train_val_gap < gap_threshold:
              ...
              best_net = net
              best_stat = stats
              print('best net so far', (lr,hs,reg,lrd) ,best_stat['val_acc_history'])
