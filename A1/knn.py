...
def compute_distances_two_loops(x_train, x_test):
  ...
  x_train_flatten = torch.flatten(x_train, start_dim=1)
  x_test_flatten = torch.flatten(x_test, start_dim=1)
  for j in torch.arange(num_test):
    for i in torch.arange(num_train):
      dists[i][j] = torch.pow((x_test_flatten[j] - x_train_flatten[i]), 2).sum() 
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return dists
...
def compute_distances_one_loop(x_train, x_test):
  ...
  for i in torch.arange(num_train):
    dists[i] = torch.pow((x_test_flatten - x_train_flatten[i]), 2).sum(dim=1)
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return dists
...
def compute_distances_no_loops(x_train, x_test):
  ...
  sqrA = torch.sum(torch.pow(A, 2), 1, keepdim=True)
  sqrA = sqrA.expand(-1, B.shape[0])

  sqrB = torch.sum(torch.pow(B, 2), 1, keepdim=True)
  sqrB = sqrB.t().expand(A.shape[0], -1)

  dists = sqrA - 2*torch.mm(A, B.t()) + sqrB
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return dists
...
def predict_labels(dists, y_train, k=1):
  ...
  ##############################################################################
  # Replace "pass" statement with your code
  Dist_sorted, Dist_sorted_idx = torch.sort(dists, 0)
  K_cloest_idx = Dist_sorted_idx[:k, :]
  #print('Dist_sorted_idx', Dist_sorted_idx)
  #print('K_cloest_idx',K_cloest_idx)

  y_train_expand = y_train.reshape(-1, 1).expand_as(Dist_sorted_idx)
  K_cloest_label = torch.gather(y_train_expand, 0, Dist_sorted_idx)[:k, :]
  print('K_cloest_label', K_cloest_label)
  y_pred, indices = torch.mode(K_cloest_label, dim=0)
  #print('y_train_expand', y_train_expand)
  #print('y_pred', y_pred)
...
def knn_cross_validate(x_train, y_train, num_folds=5, k_choices=None):
  ...
  for k in k_choices:
    accuracy_this_k = []
    for fold_idx in torch.arange(num_folds):

      x_train_cross = torch.cat((x_train_folds[0:fold_idx]+ x_train_folds[fold_idx+1:]), dim=0)
      y_train_cross = torch.cat((y_train_folds[0:fold_idx]+ y_train_folds[fold_idx+1:]), dim=0)
    
      classifier = KnnClassifier(x_train_cross, y_train_cross)
    
      accuracy_this_k.append(classifier.check_accuracy(x_train_folds[fold_idx], y_train_folds[fold_idx], k))
    k_to_accuracies[k] = accuracy_this_k
...
def knn_get_best_k(k_to_accuracies):
  ...
  for k in k_to_accuracies:
    accuracy_mean[k] = mean(k_to_accuracies[k])

  best_k = max(accuracy_mean, key=accuracy_mean.get)
  ##############################################################################
  #                            END OF YOUR CODE                                #
  ##############################################################################
  return best_k
