...
def multiples_of_ten(start, stop):
  ...
  x = torch.tensor(multiples, dtype = torch.float64) #Constructs a tensor by copying data
  #############################################################################
  #                            END OF YOUR CODE                               #
  #############################################################################
  return x
...
def zero_row_min(x):
  ...
  #############################################################################
  # Replace "pass" statement with your code
  x_min, x_min_index = x.min(dim=1)
  y = x
  index_y_0 = torch.arange(y.shape[0])
  y[index_y_0 ,x_min_index] = 0
  #############################################################################
  #                            END OF YOUR CODE                               #
  #############################################################################
  return y
...
def batched_matrix_multiply(x, y, use_loop=True):
  ...
  res = []
  if use_loop == True:
    for batch_index in torch.arange(x.shape[0]):
      res.append(torch.mm(x[batch_index],y[batch_index])) 
    res = tuple(res) 
    z = torch.stack(res, dim=0)
...
def normalize_columns(x):
  ...
  #############################################################################
  # Replace "pass" statement with your code
  x_col_mean = x.sum(dim=0)/x.shape[0] #shape(n,)
  res = (x - x_col_mean)*(x - x_col_mean)
  print('x_col_mean',x_col_mean,x_col_mean.shape)
  print('res',res,res.shape)
  res = res.sum(dim=0)#shape(n,)
  print('res',res,res.shape)
  x_col_std = torch.sqrt(res/(x.shape[0]-1))
  print('x_col_std',x_col_std)
  y = x - x_col_mean
  y = y/x_col_std
  #############################################################################
  #                            END OF YOUR CODE                               #
