...
def three_layer_convnet(x, params):
  ...
  - params: A list of PyTorch Tensors giving the weights and biases for the
    ...
    - fc_w: PyTorch Tensor giving weights for the fully-connected layer. Can you
      figure out what the shape should be? (C,D)
  ...
  x = F.conv2d(x, conv_w1, conv_b1, padding=2)#same padding #H_out = (H – K + 2P) / S + 1 = (32 – 5 + 2*2) / 1 + 1 = 32
  x = F.relu(x)
  x = F.conv2d(x, conv_w2, conv_b2, padding=1)#same padding #x.shape(N,channel2,H,W)=(64,9,32,32)
  x = F.relu(x) 
  x = flatten(x)#x shape (N,D)= (64,9*32*32)
  scores = F.linear(x, fc_w, fc_b)#x.mm(w.t()) + b
...
def initialize_three_layer_conv_part2(dtype=torch.float, device='cpu'):
  ...
  ##############################################################################
  # Replace "pass" statement with your code
  D = channel_2 * H * W# after same padding conv and flatten: the output.shapep(N,D)
  ...
  fc_w = nn.init.kaiming_normal_(torch.empty((num_classes,D), dtype=dtype, device=device))
...
class ResNet(nn.Module):
  ...
  def forward(self, x):
    ...
    # add global average pooling(we can add it only in forwarld pass because there is no learnable parameters in pooling)
...
class ResidualBottleneckBlock(nn.Module):
  def __init__(self, Cin, Cout, downsample=False):
    ...
    self.block = nn.Sequential(OrderedDict([
      ('bn1', nn.BatchNorm2d(Cin)),
      ('relu1', nn.ReLU()),
      ('conv1', nn.Conv2d(Cin, Cout//4, kernel_size=1, stride=stride, padding=0)),
      ('bn2', nn.BatchNorm2d(Cout//4)),
      ('relu2', nn.ReLU()),
      ('conv2', nn.Conv2d(Cout//4, Cout//4, kernel_size = 3, stride= 1, padding=1)),
      ('bn3', nn.BatchNorm2d(Cout//4)),
      ('relu3', nn.ReLU()),
      ('conv3', nn.Conv2d(Cout//4, Cout, kernel_size = 1, stride= 1, padding=0))
    ]))


    self.shortcut = ResidualBlock(Cin, Cout, downsample).shortcut
    ############################################################################
    #                                 END OF YOUR CODE                         #
    ############################################################################
